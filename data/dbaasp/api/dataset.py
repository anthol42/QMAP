...
def _format_sample_line(sample: Sample) -> str:
    return f'{str(sample.id).ljust(10, " ")} {sample.sequence[:40].ljust(40, " ")} {(sample.nterminal or 'Free').ljust(12)} {(sample.cterminal or 'Free').ljust(12)} {str(len(sample.targets)).ljust(10)} {sample.hc50.consensus if sample.hc50 else float('nan'):.2f}'
...
class DBAASPDataset:
    ...
    def __getitem__(self, idx: int | slice | np.ndarray | list[bool]):
