...
def random_cluster_split(clusters: pd.DataFrame, test_ratio: float = 0.2, verbose: bool = True) -> tuple[list[int], list[int]]:
    """
    ...
    :param verbose: Whether to print the number of clusters found.
    :return: Two sets of ids, one for training and one for testing.
    """
    cluster_ids = clusters['community'].unique()
    print(f"Found {len(cluster_ids)} clusters in the dataset.") if verbose else None
