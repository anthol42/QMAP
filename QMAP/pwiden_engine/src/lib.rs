...
// Note: filter_out is not re-exported due to module name collision
...
#[pymodule]
fn _pwiden_engine(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(filter_out::filter_out, m)?)?;
